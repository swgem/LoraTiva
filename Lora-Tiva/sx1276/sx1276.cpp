...
SX1276* g_radio_driver;

void OnTimeoutIrqWrapper0()
{
	g_radio_driver->OnTimeoutIrq();
	TimerIntClear(TIMER0_BASE, TIMER_TIMA_TIMEOUT);
}

void OnTimeoutIrqWrapper1()
{
	g_radio_driver->OnTimeoutIrq();
	TimerIntClear(TIMER1_BASE, TIMER_TIMA_TIMEOUT);
}

void OnTimeoutIrqWrapper5()
{
	g_radio_driver->OnTimeoutIrq();
...
void ConfigureTimeoutTimers()
{
    //Timeout txTimeoutTimer; - timer5
    //Timeout rxTimeoutTimer; - timer0
    //Timeout rxTimeoutSyncWord; - timer1
...
void TimeoutAttachUs(uint32_t ui32Base, void (*callback)(),float time_us)
{
	float ticks = SysCtlClockGet()*time_us/(1e6);
	if(ticks > (float)0xFFFFFFFF)
	{
		ticks = (float)0xFFFFFFFF;
	}
	TimerLoadSet(ui32Base,TIMER_A,(uint32_t)ticks);
	TimerIntRegister(ui32Base, TIMER_A ,callback);
	TimerEnable(ui32Base,TIMER_A);
}

void TimeoutDettachUs(uint32_t ui32Base)
{
	TimerDisable(ui32Base,TIMER_A);
}

void OnDio0IrqWrapper()
{
	g_radio_driver->OnDio0Irq();
}

void OnDio1IrqWrapper()
{
	g_radio_driver->OnDio1Irq();
}

void OnDio2IrqWrapper()
{
	g_radio_driver->OnDio2Irq();
}

void OnDio3IrqWrapper()
{
	g_radio_driver->OnDio3Irq();
}

void OnDio4IrqWrapper()
{
	g_radio_driver->OnDio4Irq();
}

uint8_t gRCVBuffer[RX_BUFFER_SIZE];
                ...
                isRadioActive( false )*/
{
	...
	this->rxTx = 0;
    this->rxtxBuffer = gRCVBuffer;
    previousOpMode = RF_OPMODE_STANDBY;
    
    ConfigureTimeoutTimers();
    this->RadioEvents = events;
    
    //this->dioIrq = new DioIrqHandler[6];

    this->dioIrq[0] = OnDio0IrqWrapper;
    this->dioIrq[1] = OnDio1IrqWrapper;
    this->dioIrq[2] = OnDio2IrqWrapper;
    this->dioIrq[3] = OnDio3IrqWrapper;
    this->dioIrq[4] = OnDio4IrqWrapper;
    this->dioIrq[5] = NULL;
    
    txTimeoutTimer = TIMER5_BASE;
    txTimeoutWrapper = OnTimeoutIrqWrapper5;

    rxTimeoutTimer = TIMER0_BASE;
    rxTimeoutWrapper = OnTimeoutIrqWrapper0;

    rxTimeoutSyncWord = TIMER1_BASE;
    rxTimeoutSyncWordWrapper = OnTimeoutIrqWrapper1;
...
{
	TimeoutDettachUs(txTimeoutTimer);
	TimeoutDettachUs(rxTimeoutTimer);
...
void SX1276::Rx( uint32_t timeout )
{
    ...
    case MODEM_LORA:
        {
            ...
            else
            {
                ...
                // DIO0=RxDone, DIO3=RFLR_IRQFLAGS_VALIDHEADER
                Write( REG_DIOMAPPING1, ( Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK & RFLR_DIOMAPPING1_DIO3_MASK ) | RFLR_DIOMAPPING1_DIO0_00 | RFLR_DIOMAPPING1_DIO3_01 );
            }
    ...
    if( timeout != 0 )
    {
    	TimeoutAttachUs(rxTimeoutTimer,rxTimeoutWrapper, timeout);
    ...
    if( this->settings.Modem == MODEM_FSK )
    {
        ...
        {
        	TimeoutAttachUs(rxTimeoutSyncWord,rxTimeoutSyncWordWrapper, ( 8.0 * ( this->settings.Fsk.PreambleLen +
                    ( ( Read( REG_SYNCCONFIG ) &
...
void SX1276::Tx( uint32_t timeout )
{
    ...
    TimeoutAttachUs(txTimeoutTimer,txTimeoutWrapper, timeout);
...
void SX1276::OnTimeoutIrq( void )
{
    ...
    case RF_RX_RUNNING:
        if( this->settings.Modem == MODEM_FSK )
        {
            ...
            {
                this->settings.State = RF_IDLE;
                TimeoutDettachUs(rxTimeoutSyncWord);
